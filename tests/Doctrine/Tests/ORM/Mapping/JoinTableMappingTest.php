...
final class JoinTableMappingTest extends TestCase
{
    public function testItSurvivesSerialization(): void
    {
        ...
        $mapping->quoted             = true;
        $mapping->joinColumns        = [JoinColumnMapping::fromMappingArray(['name' => 'id', 'referencedColumnName' => 'id'])];
        $mapping->inverseJoinColumns = [JoinColumnMapping::fromMappingArray(['name' => 'id', 'referencedColumnName' => 'id'])];
        $mapping->schema             = 'foo';
